...
当前已经完成 **T0–T6 的独立基础模块**：数据模型、屏幕抓取、冻结框选、离线 OCR、SQLite/FTS5 本地卡片存储、Chrome MV3 扩展与本机 WebSocket 桥，以及一次框选生成卡片的流水线。T1/T2 的真实多显示器和高 DPI 效果仍需要在用户的 Windows 桌面人工验收；T5 的协议自动测试已经通过，仍需在真实 Chrome 中加载扩展完成人工验收；T6 的 DOM/OCR、截图、落库与失败回滚已通过隔离目录集成测试。
...
确认界面和搜索服务会按 [开发路线图](docs/roadmap.md) 分阶段加入。当前主入口仍只完成最小启动检查；T1–T6 已经能够由代码组合调用，但还没有用户可见的全局快捷键和确认窗口，因此不代表完整采集闭环已经可用。
...
当前入口只用于验证包结构和配置能够正常加载。T1–T6 已提供独立抓帧、框选、浏览器上下文、OCR、截图和落库能力；完整的“快捷键 → 冻结 → 框选 → 确认观点卡片”用户流程将在 T7 串联。
...
流水线目前是可测试的内部 API。T7 会负责在后台线程中调用它，并加入快捷键、托盘、文字校对、明确态度、保存与丢弃界面；在 T7 完成前，不建议把当前入口当作日常可用 App。
