...
当前版本处于 **T0：项目初始化与数据模型**。仓库已经加入目录骨架、集中配置、`Card` 数据契约和对应基础测试；截屏、OCR、存储与界面仍按后续任务推进。
...
截屏、框选、OCR、存储、浏览器桥接和搜索服务会按 [开发路线图](docs/roadmap.md) 分阶段加入。当前入口可能只完成最小启动检查，不代表完整采集闭环已经可用。
...
T0 阶段的入口只用于验证包结构和配置能够正常加载。完整的“快捷键 → 冻结 → 框选 → 观点卡片”流程会在 T1、T2、T6 和 T7 完成。
