...
当前已完成 **T0：项目初始化与数据模型**，并完成 **T1/T2 的代码实现**：程序能够捕获鼠标所在显示器、记录 DPI 和前台应用元数据，并在冻结画面上进行主动框选。T1/T2 的纯逻辑与无界面冒烟测试已经通过，真实多显示器和高 DPI 效果仍需要在用户的 Windows 桌面人工验收。
...
OCR、存储、浏览器桥接和搜索服务会按 [开发路线图](docs/roadmap.md) 分阶段加入。当前主入口仍只完成最小启动检查；T1/T2 需要通过独立人工脚本体验，不代表完整采集闭环已经可用。
...
当前入口只用于验证包结构和配置能够正常加载。T1/T2 已提供独立抓帧与框选能力；完整的“快捷键 → 冻结 → 框选 → 观点卡片”流程仍需在 T6 和 T7 串联。
