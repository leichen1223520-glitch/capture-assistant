...
当前已经完成 **T0–T4 的独立基础模块**：数据模型、屏幕抓取、冻结框选、离线 OCR，以及 SQLite/FTS5 本地卡片存储。T1/T2 的纯逻辑与无界面冒烟测试已经通过，真实多显示器和高 DPI 效果仍需要在用户的 Windows 桌面人工验收；T3 已使用内存生成的固定中英文样图完成真实 RapidOCR 验证，T4 已完成临时数据库 CRUD、检索和截图删除验证。
...
浏览器桥接、卡片生成流水线、确认界面和搜索服务会按 [开发路线图](docs/roadmap.md) 分阶段加入。当前主入口仍只完成最小启动检查；T1–T4 是可独立调用和测试的模块，不代表完整采集闭环已经可用。
...
当前入口只用于验证包结构和配置能够正常加载。T1–T4 已提供独立抓帧、框选、OCR 和存储能力；完整的“快捷键 → 冻结 → 框选 → 观点卡片”流程仍需在 T5–T7 串联。
